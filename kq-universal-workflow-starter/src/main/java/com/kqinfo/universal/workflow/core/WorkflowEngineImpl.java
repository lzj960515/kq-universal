...
public class WorkflowEngineImpl implements WorkflowEngine, ApplicationListener<ApplicationReadyEvent> {
    ...
    public void initProcess(ProcessDefConfig processDefConfig) {
        doInitProcess(JSONUtil.parseObj(processDefConfig));
    }
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public Integer rejectToPreNode(String processName, String businessId, String operator, String reason) {
        // 将当前任务完成
        final Task task = taskService.complete(processName, businessId, operator, TaskStatusEnum.REJECT.value(), reason);
        final Execution execution = getExecution(task, null);
        // 查找上一个节点
        ...
        if (parentId == null || parentId == 0) {
            ...
            // 将任务回退上一个
            taskService.rollbackToPreTask(task);
        }
        // 任务拒绝后执行对应的事件
        // 取到当前任务的节点
        final WorkNode node = workflowHandler.getNode(task.getName(), execution.getWorkNodes());
